...
begin
  try
    if Sqlite3_Prepare_v2(Self.fDB, PAnsiChar(AnsiString(SQL)), -1, Stmt, NextSQLStatement) <> SQLITE_OK then
...
begin
  ...
  if Sqlite3_Prepare(self.fDB, PAnsiChar(AnsiString(SQL)), -1, Stmt, NextSQLStatement) <> SQLITE_OK then
    RaiseError('Error executing SQL', SQL)
  else
...
begin
  ...
  Self.fColCount := 0;
  Self.fSQL := SQL;
  if SQLite3_Prepare_v2(DB.fDB, PAnsiChar(AnsiString(SQL)), -1, fStmt, NextSQLStatement) <> SQLITE_OK then
    DB.RaiseError('Error executing SQL', SQL);
  if fStmt = nil then
    DB.RaiseError('Could not prepare SQL statement', SQL);
  DB.DoQuery(SQL);
