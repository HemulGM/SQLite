...
type
  ESQLiteInitException = class(Exception)end;
...
type
  ...
  //Прототип функции для определения собственной сортировки
  ...
  TSQLite3_Create_Function  = function(DB:TSQLiteDB; zFunctionName:MarshaledAString; nArg, eTextRep:Integer; pApp:Pointer; xFunc:TxFunc; xStep:TxStep; xFinal:TxFinal):Integer; cdecl;
  TSQLite3_Value_Text       = function(Value:Pointer):MarshaledAString; cdecl;
  TSQLite3_Result_Text      = procedure(Context:Pointer; Data:MarshaledAString; Size:Integer; Proc:TResultDestructor); cdecl;
...
////
//В запросах SQL, вводимых в sqlite3_prepare() и sqlite3_prepare_v2(),
//один или несколько литералов могут быть заменены шаблоном "?" Или ":N:" где N - целое число.
//Эти значения быть установлены с помощью методов sqlite3_bind_*.
//
//В каждом случае первым параметром является указатель на структуру sqlite3_stmt, возвращенную из sqlite3_prepare().
//Второй параметр - это индекс подстановочного знака.
//Первый "?" Имеет индекс 1. ":N:" подстановочные знаки используют индекс N.
////
//Пятый параметр для sqlite3_bind_blob(), sqlite3_bind_text() является деструктором,
//используемым для удаления BLOB или text после завершения работы SQLite с ним.
//Если пятый аргумент представляет собой специальное значение SQLITE_STATIC,
//библиотека предполагает, что информация находится в статическом,
//неуправляемом пространстве и не нуждается в освобождении.
//Если пятый аргумент имеет значение SQLITE_TRANSIENT, тогда SQLite создает
//свою собственную личную копию данных.
////
//Методы sqlite3_bind_* должны быть вызваны перед sqlite3_step(),
//после sqlite3_prepare() или sqlite3_reset().
//Несопоставленные подстановочные знаки интерпретируются как NULL.
////
...
begin
 case SQLiteFieldTypeCode of
  ...
  Result:='Неизвестный код поля SQLite "'+IntToStr(SQLiteFieldTypeCode)+'"';
 end;
end;
...
begin
 case SQLiteErrorCode of
  SQLITE_OK:         Result:='Успешный результат';
  SQLITE_ERROR:      Result:='Ошибка SQL или отсутствует база данных';
  SQLITE_INTERNAL:   Result:='Внутренняя логическая ошибка SQLite';
  SQLITE_PERM:       Result:='Отказано в доступе';
  SQLITE_ABORT:      Result:='Процедура обратного вызова запросила отмену';
  SQLITE_BUSY:       Result:='Файл базы данных заблокирован';
  SQLITE_LOCKED:     Result:='Таблица в базе данных заблокирована';
  SQLITE_NOMEM:      Result:='Ошибка malloc()';
  SQLITE_READONLY:   Result:='Попытка записи в базу данных только для чтения';
  SQLITE_INTERRUPT:  Result:='Операция завершена sqlite3_interrupt()';
  SQLITE_IOERR:      Result:='Произошла ошибка дискового ввода-вывода';
  SQLITE_CORRUPT:    Result:='Образ диска базы данных имеет неверный формат';
  SQLITE_NOTFOUND:   Result:='(Только для внутреннего использования) Таблица или запись не найдены';
  SQLITE_FULL:       Result:='Ошибка при вставке, поскольку база данных заполнена';
  SQLITE_CANTOPEN:   Result:='Не удалось открыть файл базы данных';
  SQLITE_PROTOCOL:   Result:='Ошибка протокола базы данных';
  SQLITE_EMPTY:      Result:='База данных пуста';
  SQLITE_SCHEMA:     Result:='Схема базы данных изменена';
  SQLITE_TOOBIG:     Result:='Слишком много данных для одной строки таблицы';
  SQLITE_CONSTRAINT: Result:='Прервано из-за нарушения ограничения';
  SQLITE_MISMATCH:   Result:='Несовпадение типа данных';
  SQLITE_MISUSE:     Result:='Библиотека используется неправильно';
  SQLITE_NOLFS:      Result:='Использует функции ОС, не поддерживаемые на хосте';
  SQLITE_AUTH:       Result:='Авторизация запрещена';
  SQLITE_FORMAT:     Result:='Ошибка форматирования вспомогательной базы данных';
  SQLITE_RANGE:      Result:='Второй параметр для sqlite3_bind вне диапазона';
  SQLITE_NOTADB:     Result:='Открыт файл, который не является файлом базы данных';
  SQLITE_ROW:        Result:='Sqlite3_step() имеет еще одну строку';
  SQLITE_DONE:       Result:='Выполнение sqlite3_step() завершено';
 else
...
begin
 if Value = nil then Result:='NULL' else Result:=Value;
end;

procedure InitSQL;
begin
 SQLiteDLLInstance:=LoadLibrary(SQLiteDLL);
 if SQLiteDLLInstance <> 0 then
  ...
  begin
   ShowMessage('Файл не найден '+SQLiteDLL);
   Halt(0);
  end;
end;
